...
def detect_objects(img_path, conf=0.5, iou=0.5):
    ...
    # Rescale the image to the input size expected by the model
    img = orig_img.resize((input_details[0]['shape'][1], input_details[0]['shape'][2]))
    
    # Preprocess the image (Normalization and Quantization)
    ...
    # Check the shape of num_detections and handle accordingly
    print(f"num_detections shape: {num_detections_tensor.shape}")
    
    if num_detections_tensor.size == 1:
        num_detections = num_detections_tensor.item()  # If only one element
    else:
        print(f"Unexpected size for num_detections_tensor: {num_detections_tensor.shape}")
        num_detections = num_detections_tensor[0]  # Or handle appropriately

    # Visualize the detections
    visualize_detections(orig_img, boxes, classes, scores, labels, threshold=conf, iou_threshold=iou)
