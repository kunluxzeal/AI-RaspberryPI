...
model_path = "./obj_model/model_float16.tflite"
labels_path = "./obj_model/label2.txt"
...
def load_model():
    ...
    if interpreter is None:
        ...
        print("Number of outputs:", len(interpreter.get_output_details()))
    return interpreter
...
def detect_objects(img, interpreter):
    ...
    # Get outputs - adjust based on your actual model output
    if len(output_details) == 1:
        # Classification model (single output with class probabilities)
        predictions = interpreter.get_tensor(output_details[0]['index'])
        return None, None, predictions[0], None  # boxes, classes, scores, num_detections
    else:
        # Object detection model (multiple outputs)
        ...
        if is_detecting:
            try:
                ...
                if current_frame is not None:
                    ...
                    if scores is not None:  # Classification model
                        # scores contains class probabilities
                        ...
                        new_detections.append({
                            'class': class_name,
                            'score': confidence,
                            'box': [0, 0, img.width, img.height]  # Full image for classification
                        })
                    else:  # Object detection model
