...
tracked_objects = {}  # Dictionary to store tracked objects {id: object_data}
next_object_id = 1    # Counter for assigning unique IDs
MAX_FRAMES_MISSED = 5  # Remove objects not seen for this many frames
...
def detection_worker():
    global frame, is_detecting, latest_detections , tracked_objects, next_object_id
        ...
        if is_detecting:
            try:
                ...
                if current_frame is not None:
                    ...
                    for i in range(int(num_detections)):
                        if scores[i] >= confidence_threshold:
                            ...
                            for obj_id, obj in tracked_objects.items():
                                prev_left, prev_top, prev_right, prev_bottom = obj['box']
                                
                                if(left < prev_right and right > prev_left and top < prev_bottom and bottom > prev_top):
                                    matched_id = obj_id
                                    break
                            ...
                            detection_data = {
                                ...
                                'timestamp':time.time(),
                                'frmaes_missed' : 0  #Reset missed frame counter
                                
                            }
                            if matched_id is not None:
                                
                                if 'frames_missed' not in tracked_objects[matched_id]:
                                    tracked_objects[matched_id]['frames_missed'] = 0
                                    
                                tracked_objects[matched_id] = detection_data
                                matched_ids.add(matched_id)
                                
                            else:
                                #New object
                                tracked_objects[next_object_id] = detection_data
                    ...
                    #Second pass: Update missed objects and remove stale ones
                    
                    stale_ids = []
                    for obj_id, obj in tracked_objects.items():
                        if obj_id not in matched_ids:
                            #Ensure  frames_missed  exists before incrementing
                            if 'frames_missed' not in obj:
                                tracked_objects[obj_id]['frames_missed'] = 0
                            tracked_objects[obj_id]['frames_missed'] += 1 
                            
                            if tracked_objects[obj_id]['frames_missed'] > MAX_FRAMES_MISSED:
